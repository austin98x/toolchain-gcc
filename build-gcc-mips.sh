...
function build_glibc()
{
	...
	dir=`package_dir ${_PACKAGE} .tar.xz`
	dir_p=`package_dir ${_PACKAGE_P} .tar.bz2`
	...
	if [ ! -d "$dir" ] ; then
		...
		prepare_package ${_PACKAGE_P} http://ftp.gnu.org/gnu/glibc
		mv $dir_p $dir/ports
	fi
...
function usage()
{
	...
	echo "Options:"
	echo "-i                target abi, [32]"
	echo "-a                target arch, [mips3]"
	echo "-t                target, [mips64el-unknown-linux-gnu]"
